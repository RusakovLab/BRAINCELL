...
NEURON {
    SUFFIX  GluTrans
    USEION glu READ gluo, glui WRITE iglu VALENCE -1
...
ASSIGNED {
	...
	nai   (mM)
	ki   (mM)
    itrans (mA/cm2)            : kept for internal calculation
    iglu   (mA/cm2)            : WRITE to glu ion - negative = uptake from ECS
    surf   (cm2)
    volin  (liter)
    volout (liter)
...
BREAKPOINT {
    ...
    : iglu is the current seen by _InsideOutDiffHelper.
    : Uptake removes glutamate from ECS: the transporter runs forward (itrans < 0),
    : so iglu must also be negative to produce a negative flux into specieso.
    : We keep the sign: iglu = itrans (both negative during forward uptake).
    iglu = itrans
}
