...
COMMENT
    Outside-In Diffusion Helper (membrane mechanism)
    
    Its job is to calculate and write "{species}o" based on the extracellular sources created by user with the Manager of Extracellular Sources
    
    If you want to add new species (or delete old ones), look for 7 blocks of code below marked as "Edit here when changing the species list"
    Warning: Any changes to the blocks must be done synchronously with the changes to two other files:
             ...
             * the file "Mechanisms\Common\MOD_files\_InsideOutDiffHelper.mod"
...
:   * when variable time step is used, it's important to schedule calls of our code in this MOD file
:     on each "on-off" event and each "spike" event of each extracellular source
:   * investigate whether enlisting all available ions here causes significant performance or memory penalties
:     compared to the case when we enlist only the used ones;
:     if it does, then consider creation of individual MOD file for each ion OR generation of this single MOD file
:     from Python, then compile it and load dynamically
...
NEURON {
    ...
    :          For more info, read the warning regarding valence in "_InsideOutDiffHelper.mod".
...
PROCEDURE calcAllOutConcs() {
    ...
    VERBATIM
        ...
        *ptr_vec_o[11] = extra2oinit;
        ...
        for (int spcIdx = 0; spcIdx < NUM_SPECIES_IN_MOD; spcIdx++) {
...
VERBATIM
    ...
    double getTimeFactorTerm(double distance, double diffFactor, double delta_t, struct SpeciesInfo *ptr_spcInfo) {
        double timeFactorTerm = erfc(distance / sqrt(diffFactor * delta_t));
        if (ptr_spcInfo->isEnableUptake) {
            timeFactorTerm *= exp(-delta_t / ptr_spcInfo->t_alpha);
        }
                ...
                switch ((int)ptr_ecSrc->temporalInfo.enumStaticSwitchSpike) {
                    case 0: {       // "static" dynamics
                        ...
                        *ptr_o += ptr_ecSrc->capacityInfo.ssOrMinus1 * ptr_ecSrc->capacityInfo.pointCapacityRadiusOrMinus1 / distance;
                        ...
                        // For t == offsetTime, we would have 1/0 in getTimeFactorTerm below
                        if (t <= offsetTime) {
                            break;
                        }
                        ...
                        double timeFactor = getTimeFactorTerm(distance, diffFactor, delta_t, ptr_spcInfo);
                        
                        double endTime = offsetTime + ptr_ecSrc->temporalInfo.durationOrMinus1;
                        
                        // For t == endTime, we would have 1/0 in getTimeFactorTerm below
                        if (t > endTime) {
                            delta_t = t - endTime;
                            timeFactor -= getTimeFactorTerm(distance, diffFactor, delta_t, ptr_spcInfo);
