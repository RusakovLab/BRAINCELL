...
from OtherInterModularUtils import getAllBiophysMechNamesSet
...
class BiophysJsonFileHelper:
    ...
    def importStage1(self, inJsonFilePathName):
        ...
        recMechNamesSet = getAllBiophysMechNamesSet()       # "rec" - recipient
        
        missingMechNames = h.List()
        for mechName in donMechNames:
            if mechName not in recMechNamesSet:     # !! h.name_declared(mechName) can give a false positive
                missingMechNames.append(h.String(mechName))
                
        if missingMechNames:
            h.mwh.showWarningBox(
        ...
        h.beih.importStage2(compNames, numInhomVars, numStochVars)      # --> importStage3
