...
class GensForExtracellularDiffusion:
    ...
    def createSpeciesLibrary(self):
        if not h.exportOptions.isExportExtracellularLibrary():
            return emptyParagraphHint()
            
        lines = []
        
        speciesLibrary = h.speciesLibrary
        ...
        hocCommand2Templ = 'execute("{}oinit_OutsideInDiffHelper = {}")'
    ...
    def createSourcesLibrary(self):
        ...
        relDirPath = '_Code\\Extracellular\\OutsideInDiffusion\\ExtracellularSourcesLibrary\\Exported'
        ecslTemplNames = [
            'ECLDeploymentCache',
            'ECSSpatialInfo',
            'ECSTemporalInfo',
        ...
        lines.append('')
        newLines = getAllLinesFromFile('_Code\\Extracellular\\OutsideInDiffusion\\Utils\\ExtracellularApplicatorUtils_Exported.hoc')
    ...
    def createExtraDiffFinale(self):
        ...
        lines.append(f'{{ applyExtracellularSources({h.pyObj.ms_veryMinOuterConc}) }}')
    ...
    def _eachSpcInfoInLibExceptUnrecCat(self):
        ...
        #       for spcInfo? in h.speciesLibrary.eachSpcInfoInLibExceptUnrecCat(spcInfo?):
        # !! Keep in sync with hoc:SpeciesLibrary.eachSpcInfoInLibExceptUnrecCat
        for spcCat in h.speciesLibrary.spcCatsList:
    ...
    def _getCapacityInfoCtorArgs(self, enumPointSphere, enumStaticSwitchSpike, capacityInfo):
        ...
        # !! for ssOrMinus1, numMoleculesOrMinus1 and delta_oOrMinus1, the negative values can actually make some sense
        ...
        if enumPointSphere == 0:
            ...
            if enumStaticSwitchSpike < 2:
                ...
                args.append(ssOrMinus1)
                self._makeSureNotMinus1ThenAppend(args, pointCapacityRadiusOrMinus1)
            elif enumStaticSwitchSpike == 2:
                # self._makeSureNotMinus1ThenAppend(args, numMoleculesOrMinus1)
                args.append(numMoleculesOrMinus1)
            else:
                codeContractViolation()
        elif enumPointSphere == 1:
            if enumStaticSwitchSpike > 2:
