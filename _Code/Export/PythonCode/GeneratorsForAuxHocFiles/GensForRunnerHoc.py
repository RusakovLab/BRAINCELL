...
class GensForRunnerHoc:
    ...
    def getOutFolderAndFileNames(self):
        ...
        isRepeatForStats = h.exportOptions.isRepeatForStats
        isAnyWatchedVars = h.exportOptions.isAnyWatchedVars()
        isAnyWatchedAPCounts = h.exportOptions.isAnyWatchedAPCounts()
    ...
    def getSweptVarsAndRepetitionSettings(self):
        if not (h.exportOptions.isAnySweptVars() or h.exportOptions.isRepeatForStats):
            return emptyParagraphHint()
            
        lines = []
        
        if h.exportOptions.isAnySweptVars():
            sweptVarsList = h.exportOptions.sweptVarsList
            numSweptVars = len(sweptVarsList)
            
            if h.exportOptions.isAnyWatchedVars() or h.exportOptions.isAnyWatchedAPCounts():
        ...
        if h.exportOptions.isRepeatForStats:
            lines.append(f'numRepeatsForStats = {int(h.exportOptions.numRepeatsForStats)}')
            lines.append('')
    ...
    def getWatchedVarsAndRecorderSettings(self):
        if not (h.exportOptions.isAnyWatchedVars() or h.exportOptions.isAnyWatchedAPCounts()):
            return emptyParagraphHint()
            
        lines = []
        
        if h.exportOptions.isAnyWatchedVars():
            watchedVarsList = h.exportOptions.watchedVarsList
            ...
            if h.exportOptions.DtOrMinus1 != -1:
                units = UnitsUtils.getUnitsForWatchedVar('dt')
                lines.append('DtOrMinus1 = {}    // ({})'.format(h.exportOptions.DtOrMinus1, units))
            else:
                lines.append('DtOrMinus1 = {}'.format(int(h.exportOptions.DtOrMinus1)))
            lines.append('')
    ...
    def getMainPart(self):
        ...
        isAnyWatchedVars = h.exportOptions.isAnyWatchedVars()
        isAnyWatchedAPCounts = h.exportOptions.isAnyWatchedAPCounts()
        isExportAltRunControl = h.exportOptions.isExportAltRunControl()
